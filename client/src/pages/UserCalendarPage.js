...
function sameDayInTz(isoUtc, year, month, day, tz) {
...
function ProfileMonthView({ year, month, events, ownerTz, onDayClick }) {
    ...
    return (_jsxs("div", { children: [_jsx("div", { className: "grid grid-cols-7 text-xs font-medium text-ink-muted mb-1", children: ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(d => (_jsx("div", { className: "text-center py-1", children: d }, d))) }), _jsx("div", { className: "grid grid-cols-7 border-l border-t border-warm-border", children: cells.map((day, i) => {
                    ...
                    return (_jsx("div", { onClick: () => day && onDayClick(new Date(year, month, day)), className: `min-h-20 border-b border-r border-warm-border p-1 cursor-pointer hover:bg-coral-tint transition-colors ${!day ? 'bg-warm-card cursor-default' : ''}`, children: day && (_jsxs(_Fragment, { children: [_jsx("span", { className: `text-xs inline-flex items-center justify-center w-6 h-6 rounded-full mb-1 ${isToday ? 'bg-coral text-white font-semibold' : 'text-ink'}`, children: day }), dayEvents.map((e, idx) => {
                                    ...
                                    return (_jsx("div", { style: { backgroundColor: profileEventColor(e) }, className: `w-full text-left text-xs text-white px-1.5 py-0.5 rounded truncate mb-0.5 ${e.status === 'cancelled' ? 'opacity-40 line-through' : ''}`, children: e.allDay ? e.title : `${fmtTimeInTz(e.start, ownerTz)} ${e.title}` }, e.id + idx));
                                })] })) }, i));
                }) })] }));
}
// ── Day view ──────────────────────────────────────────────────
function ProfileDayView({ date, events, ownerTz }) {
    ...
    return (_jsx("div", { className: "space-y-2", children: dayEvents.map((e, idx) => {
            if (!e.visible) {
                return (_jsxs("div", { className: "flex items-center gap-3 p-3 rounded-card bg-warm-card border border-warm-border", children: [_jsx("span", { className: "w-2.5 h-2.5 rounded-full bg-warm-border flex-shrink-0" }), _jsxs("div", { children: [_jsx("p", { className: "text-sm font-medium text-ink-muted italic", children: "Busy" }), _jsx("p", { className: "text-xs text-ink-muted", children: e.allDay ? 'All day' : `${fmtTimeInTz(e.start, ownerTz)} – ${fmtTimeInTz(e.end, ownerTz)}` })] }), _jsx("span", { className: "ml-auto text-ink-muted text-sm", children: "\uD83D\uDD12" })] }, e.id + idx));
            }
            const color = profileEventColor(e);
            return (_jsxs("div", { className: `flex items-start gap-3 p-3 rounded-card bg-white border border-warm-border hover:bg-coral-tint transition-colors ${e.status === 'cancelled' ? 'opacity-50' : ''}`, children: [_jsx("span", { className: "w-2.5 h-2.5 rounded-full mt-1 flex-shrink-0", style: { backgroundColor: color } }), _jsxs("div", { className: "flex-1 min-w-0", children: [_jsx("p", { className: `text-sm font-medium text-ink ${e.status === 'cancelled' ? 'line-through' : ''}`, children: e.title }), _jsx("p", { className: "text-xs text-ink-muted", children: e.allDay ? 'All day' : `${fmtTimeInTz(e.start, ownerTz)} – ${fmtTimeInTz(e.end, ownerTz)}` }), e.location && _jsx("p", { className: "text-xs text-ink-muted truncate", children: e.location }), e.description && _jsx("p", { className: "text-xs text-ink-muted mt-1 line-clamp-2", children: e.description })] }), _jsx("span", { className: "text-xs text-ink-muted flex-shrink-0 capitalize", children: e.visibility })] }, e.id + idx));
        }) }));
}
// ── User calendar page ────────────────────────────────────────
export default function UserCalendarPage() {
    ...
    return (_jsxs(Layout, { children: [_jsxs("div", { className: "flex items-center gap-3 mb-4 flex-wrap", children: [_jsx("button", { onClick: () => navigate('/people'), className: "text-sm text-ink-muted hover:text-ink transition-colors", children: "\u2190 Back" }), _jsxs("h1", { className: "text-lg font-semibold text-ink", children: [userName, "'s Calendar"] }), _jsx("span", { className: "text-xs px-2 py-0.5 rounded-pill bg-coral-tint text-coral-dark", children: isFriend ? 'Friend view' : 'Public view' }), ownerTz !== 'UTC' && (_jsx("span", { className: "text-xs px-2 py-0.5 rounded-pill bg-warm-card text-ink-muted border border-warm-border", children: ownerTz }))] }), _jsxs("div", { className: "flex flex-wrap items-center justify-between gap-3 mb-4", children: [_jsx("div", { className: "flex items-center gap-2", children: view === 'month' ? (_jsxs(_Fragment, { children: [_jsx("button", { onClick: prevMonth, className: "p-1.5 rounded-card hover:bg-warm-card text-ink-muted text-lg transition-colors", children: "\u2039" }), _jsx("h2", { className: "text-base font-semibold text-ink w-44 text-center", children: fmtMonth(year, month) }), _jsx("button", { onClick: nextMonth, className: "p-1.5 rounded-card hover:bg-warm-card text-ink-muted text-lg transition-colors", children: "\u203A" })] })) : (_jsxs(_Fragment, { children: [_jsx("button", { onClick: prevDay, className: "p-1.5 rounded-card hover:bg-warm-card text-ink-muted text-lg transition-colors", children: "\u2039" }), _jsx("h2", { className: "text-base font-semibold text-ink w-64 text-center", children: fmtDay(selected) }), _jsx("button", { onClick: nextDay, className: "p-1.5 rounded-card hover:bg-warm-card text-ink-muted text-lg transition-colors", children: "\u203A" })] })) }), _jsxs("div", { className: "flex rounded-pill border border-warm-border overflow-hidden text-sm", children: [_jsx("button", { onClick: () => setView('month'), className: `px-3 py-1.5 transition-colors ${view === 'month' ? 'bg-coral text-white' : 'bg-white text-ink-muted hover:bg-warm-card'}`, children: "Month" }), _jsx("button", { onClick: () => { setSelected(today); setView('day'); }, className: `px-3 py-1.5 transition-colors ${view === 'day' ? 'bg-coral text-white' : 'bg-white text-ink-muted hover:bg-warm-card'}`, children: "Day" })] })] }), _jsxs("div", { className: "flex items-center gap-4 mb-4 text-xs text-ink-muted", children: [_jsxs("span", { className: "flex items-center gap-1.5", children: [_jsx("span", { className: "w-3 h-3 rounded inline-block", style: { backgroundColor: '#a855f7' } }), " Visible event"] }), _jsxs("span", { className: "flex items-center gap-1.5", children: [_jsx("span", { className: "w-3 h-3 rounded bg-warm-border inline-block" }), " Private / restricted"] })] }), loading ? (_jsx("div", { className: "text-center py-20 text-sm text-ink-muted", children: "Loading calendar\u2026" })) : view === 'month' ? (_jsx(ProfileMonthView, { year: year, month: month, events: events, ownerTz: ownerTz, onDayClick: d => { setSelected(d); setView('day'); } })) : (_jsx(ProfileDayView, { date: selected, events: events, ownerTz: ownerTz }))] }));
}
